...
| API | Return | Notes |
| --- | --- | --- |
| `std.str.reverse(buffer, text)` | `Maybe<Span<u8>>` | Writes reversed bytes into caller-provided storage. |
...
- `reverse` writes into caller storage and returns `null` when the buffer is too small.
